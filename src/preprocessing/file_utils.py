...
def process_viirs_filename(file_path: Path) -> Tuple[str, str]:
    ...
    band = 0
    date = datetime.strptime(file_name[13:-4], '%Y%j')
    return date.strftime("%Y-%m-%d"), str(band)
...
def process_s1_filename(file_path: Path) -> Tuple[str, str]:
    ...
    """
    file_name = os.path.basename(file_path)
    date, band = file_name[12:-4].split('_')
...
def process_s2_filename(file_path: Path) -> Tuple[str, str]:
    ...
    """
    file_name = os.path.basename(file_path)
    date, band = file_name[4:-4].split('_')
    date = datetime.strptime(date, '%Y%m%d')
    return date.strftime("%Y-%m-%d"), str(band[1:])
...
def process_landsat_filename(file_path: Path) -> Tuple[str, str]:
    ...
    """
    file_name = os.path.basename(file_path)
    date, band = file_name[10:-4].split('_')
    date = datetime.strptime(date, '%Y-%m-%d')
    return date.strftime("%Y-%m-%d"), str(band[1:])
...
def get_satellite_files(tile_dir: Path, satellite_type: SatelliteType) -> List[Path]:
    ...
    for file in tile_dir.iterdir():
        file_name = file.name
        if pattern in file_name and ".tif" == file_name[-4:] and file.is_file():
            files.append(file)
    return files
...
def read_satellite_file(satellite_file: Path) -> np.ndarray:
    ...
    """
    tif = tifffile.imread(satellite_file)
    return tif
...
def load_satellite(tile_dir: Path, satellite_type: SatelliteType) -> xr.DataArray:
    ...
    for date in dates:
        ...
        for band in bands:
            ...
            for file in file_names:
                ...
                current_date, current_band = grouping_function(file)
    ...
    xr_data = xr.DataArray(
        final_data,
        coords={"date": dates, "band": bands, "height": range(final_data.shape[2]),
                "width": range(final_data.shape[3])},
        dims=("date", "band", "height", "width"),
        # add the satellite_type, tile_dir, and parent_tile_id as attributes to the xarray
        attrs={"satellite_type": satellite_type.value, "tile_dir": str(tile_dir), "parent_tile_id": parent_tile_id}
    )
    return xr_data.astype(float)
...
def create_satellite_dataset_list(
    ...
    for index, data_dict in enumerate(data_dict_list):
        ...
        xr_dataset = xr.Dataset(data_dict, attrs={"satellite_type": satellite_type_list[index].value})
        # append the dataset to the data list
        data_sets.append(xr_dataset)
