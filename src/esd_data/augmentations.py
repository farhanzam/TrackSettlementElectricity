...
class Blur(object):
    ...
    def __call__(self, sample: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        blurred = apply_per_band(img, lambda x: cv2.blur(x, ksize=(kernel, kernel)))

        # return the {X : img, y : mask}
        return {"X": blurred, "y": mask}
...
class AddNoise(object):
    ...
    def __call__(self, sample):
        ...
        dev = random.uniform(0, self.std_lim)
        ...
        img_with_noise = np.random.normal(self.mean, dev, img.shape) + img
        ...
        # return the {X : img, y : mask}
        return {"X": img_with_noise, "y": mask}
...
class RandomVFlip(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() <= self.p:
            ...
            flipped_X = apply_per_band(img, lambda x: cv2.flip(x, flipCode=0))

            # also flip the mask (not per band, just directly on the mask)
            flipped_y = cv2.flip(mask, flipCode=0)


        # return the {X : img, y : mask}
        return {"X": flipped_X, "y": flipped_y}
...
class RandomHFlip(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() <= self.p:
            # apply per band the cv2.flip function (you can pass it as a lambda)
            flipped_X = apply_per_band(img, lambda x: cv2.flip(x, flipCode=1))

            # also flip the mask (not per band, just directly on the mask)
            flipped_y = cv2.flip(mask, flipCode=1)
