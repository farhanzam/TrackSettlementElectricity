...
class ESDDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        if not self.processed_dir.exists() or (not list(self.processed_dir.iterdir())):
            ...
            directories = list(self.raw_dir.glob('*'))
            ...
            if self.train_size == 1:
                tile_dirs_train, tile_dirs_val = directories, []
            else:
            ...
            for td in tqdm(list(tile_dirs_train), desc="Processing train tiles"):

                # get the data array list and 
                #  data array from load_and_preprocess
                array_list, gt_array = self.load_and_preprocess(td)

                # create a subtile, passing the data array list, gt data array, and the slice size
                new_subtile = Subtile(array_list, gt_array, self.slice_size)

                # save the subtile to the train_dir
                new_subtile.save(self.train_dir)


            # iterate over the tile directories in tqdm(list(tile_dirs_val), desc="Processing validation tiles"):
            for vd in tqdm(list(tile_dirs_val), desc="Processing validation tiles"):

                # get the data array list and gt data array from load_and_preprocess
                array_list, gt_array = self.load_and_preprocess(vd)

                # create a subtile, passing the data array list, gt data array, and the slice size
                new_subtile = Subtile(array_list, gt_array, self.slice_size)

                # save the subtile to the val_dir
                new_subtile.save(self.val_dir)
    ...
    def setup(self, stage: str) -> None:
        ...
        if stage == "fit":
            ...
            self.val_dataset = ESDDataset(self.train_dir, self.transform, self.satellite_type_list, self.slice_size)
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        # and collate_fn
        return torch.utils.data.DataLoader(self.train_dataset, self.batch_size, collate_fn=collate_fn)
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        # and collate_fn
        return torch.utils.data.DataLoader(self.val_dataset, self.batch_size, collate_fn=collate_fn)
    ...
    def test_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        # create the torch.utils.data.Dataloader for the train_dataset, passing the batch size
        # and collate_fn
        return torch.utils.data.DataLoader(self.test_dataset, self.batch_size, collate_fn=collate_fn)