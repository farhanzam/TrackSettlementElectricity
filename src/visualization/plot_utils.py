...
def flatten_dataset(data_set: xr.DataArray) -> np.ndarray:
    ...
    """
    da = data_set.to_dataarray()
    size = 1
    for n in da.shape:
        size *= n
    return np.reshape(da.values, (size,))
...
def flatten_dataset_by_band(data_set: xr.DataArray) -> np.ndarray:
    ...
    """
    da = data_set.to_dataarray()
    size = da.shape[0] * da.shape[1] * da.shape[3] * da.shape[4]
    band_size = da.shape[2]
    band_first = np.transpose(da.values, (2, 0, 1, 3, 4))
    return np.reshape(band_first, (band_size, size))
...
def plot_sentinel1_histogram(
    ...
    for i in range(num_bands):
        ...
        # plot histogram
        ax[i][0].hist(flattened_dataset_list[i], bins=n_bins)
        ...
        ax[i][0].set_title(f"Sentinel 1 Histogram (log scaled)\n{num_tiles} tiles, {num_dates} dates\nBand {satellite_bands[i]}", fontsize=9)
...
def plot_landsat_histogram(
    ...
    for i in range(num_bands):
        ...
        ax[i][0].tick_params(axis="both", labelsize=9)

        # set title w/ font size 9 to: "Landsat 8 Histogram (log scaled)\n{num_tiles} tiles, {num_dates} dates\nBand {satellite_bands[band]}"
        ax[i][0].set_title(f"Landsat 8 Histogram (log scaled)\n{num_tiles} tiles, {num_dates} dates\nBand {satellite_bands[i]}", fontsize=9)
    # --- end ---
...
def plot_gt_histogram(
    ...
    ax[0][0].set_title(f"GT Histogram (log scaled), {num_tiles} tiles")
...
def plot_viirs(data_array: xr.DataArray, image_dir: Path = None) -> None:
    ...
    for i in range(len(ax)):
        ...
        for j in range(len(ax[i])):
            ...
            if d < num_dates:
                # show the image
                ax[i][j].imshow(data_array[d][b])
               
                # set title w/ font size 9 to: "VIIRS Image\n{data_array['date'][viirs_date_index].values}"
                ax[i][j].set_title(f"VIIRS Image\n{data_array['date'][d].values}")
    ...
    for index, bands in enumerate(bands_to_plot):
        ...
        for d in range(num_dates):
            ...
            for band in bands:
                ...
                data_list.append(data_array.sel(band=band).isel(date=d))
                
            # stack and transpose the data list, then imshow
            final_array = np.stack(data_list, -1)
            ax[d][index].imshow(final_array)
            
            # set the axis to off
            ax[d][index].set_axis_off()
            ...
            ax[d][index].set_title(f"{str(data_array.attrs['satellite_type']).capitalize()} {bands_for_title}\n{(data_array['date'])[d].values}", fontsize=9)
